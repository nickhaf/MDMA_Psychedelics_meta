...
The test of moderators checks the assumption that $\mu_{mdma} = \mu_{psych} = 0$, so it tests for an overall effect of the variable `drugType`. 
...
## $I^2$
How much heterogeneity is due to differences within studies (level 2), and how much due to between study-differences (level 3)?
