...
https://www.metafor-project.org/doku.php/tips:forest_plot_with_exact_cis
```{r}
...
my_dat[[1]] <- my_dat[[1]] %>%
  ...
  mutate(es.id = as.numeric(as.factor(es.id))) %>%
  arrange(-es.id) %>%
  mutate(background_colour = c(rep(c("grey90", "white"), 8), "grey90")) %>%
  dplyr::select(Study, Test, drugType, N, es, es.id, ci_lb, ci_ub, background_colour) %>%
  add_row(es.id = 0) %>%
  arrange(-es.id)


ggplot(data = my_dat[[1]], 
       mapping = aes(x = es, 
                     y = as.numeric(as.factor(es.id)), 
                     colour = drugType)) +
  ggplot2::geom_tile(
        ggplot2::aes(
          width = Inf,
          height = 1        ),
        colour = NA, 
        fill = my_dat[[1]]$background_colour
      ) +
      geom_vline(xintercept = 0, colour = "black", linetype = "dotted") +
  geom_segment(aes(x = ci_lb, 
                   xend = ci_ub,
                   y= es.id + 1, 
                   yend = es.id +1),
               colour = linecolour)+
  ...
  geom_text(aes(label = round(es, 2)),
            ...
            nudge_y = 0.3) +
  xlim(c(-17, 2)) +
  geom_text(aes(label = Study, x = -17), 
            colour = "black", 
            hjust = 0, 
            size = 3.5
            ) +
    geom_text(aes(label = Test, x = -10), 
            colour = "black", 
            hjust = 0, 
            size = 3.5
            ) +
  ...
  #           size = 2.5, 
  #           nudge_x =  0.45) +
  ggplot2::annotate("segment", x = -Inf, xend = Inf,  max(my_dat[[1]]$es.id) + 1.5, yend = max(my_dat[[1]]$es.id) + 1.5, linewidth = 0.1) +
  theme_classic() +
  theme(axis.line = element_blank(), 
        axis.ticks.y = element_blank(),
        axis.title = element_blank(), 
        axis.text.y = element_blank()) +
  scale_x_continuous(breaks=seq(-6,2,1), expand = c(0, 0.2)) +
  scale_y_continuous(expand= c(0, 0)) +
  scale_size_continuous(range = c(1, 3)) +
  annotate("segment", x = -6, xend = 2, y = 0, yend = 0) +
  annotate("text", x = -10, y = 1, label = "Effect MDMA") +
  NULL
