...
fit_rma.mv <- function(dat_list, ...){
  fitted_models <- lapply(dat_list, function(df){
  rma.mv(yi = es,
...
lapply(my_dat, function(df){
  sensitivity_cols <- colnames(df)[grep("sensitivity", colnames(df))]
  
  sensitivity_models <- lapply(sensitivity_cols, function(col){
    fit_rma.mv(df, subset = col)  
  })
  
  return(sensitivity_models)
})
